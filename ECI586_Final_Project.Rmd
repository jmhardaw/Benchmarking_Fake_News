...
subject_tokens %>%
  ...
  scale_y_discrete(limits = c("pants-fire", 
                            ...
                            "mostly-true",
                            "TRUE")) +
  labs(y = "Truth-O-Meter", x = "Statement Count", subtitle = "Truth by Issue")
```
...
```{r}
top_parties <- c("democrat", "republican", "independent", "libertarian")

liar_data %>% 
  filter(Party %in% top_parties) %>% 
  ggplot(aes(y = Label, fill = Party)) +
  geom_bar(show.legend = FALSE) +
  facet_wrap(~ Party) +
  scale_y_discrete(limits = c("pants-fire", 
                            "FALSE",
                            ...
                            "mostly-true",
                            "TRUE")) +
  labs(y = "Truth-O-Meter", x = "Statement Count", subtitle = "Truth by Party")
```
...
This graph confirms the high use of "trust" words as a potential mask to issues or discussions that aren't always truthful. The highest rating of false statements (think yellow, orange, red), are in the sentiment categories of ***trust***, ***negative***, ***positive***, and ***fear***.
